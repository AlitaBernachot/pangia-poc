...
or the per-agent configuration resolver in :mod:`app.agent.model_config`.
...
4. Add the provider's credentials to ``config.py`` and handle them inside
   :func:`~app.agent.model_config.get_agent_model_config`.
"""
...
try:
    from langchain_google_vertexai import GemmaLocalKaggle  # type: ignore[import]
...
def build_llm(config: "ModelConfig", *, streaming: bool = False) -> BaseChatModel:
    ...
    ----------
    config:
        A :class:`~app.agent.model_config.ModelConfig` specifying the
        provider, model, and connection details.
    streaming:

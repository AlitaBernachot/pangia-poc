...
All statements use MERGE / CREATE … IF NOT EXISTS / ON CONFLICT DO NOTHING
so the seeding is idempotent: running it a second time does not create
duplicates.
"""
import logging
...
# ---------------------------------------------------------------------------
# PostGIS – Spatial SQL seed
# Each statement is executed independently.  Tables are created only if they
# do not already exist; inserts use ON CONFLICT DO NOTHING for idempotency.
# ---------------------------------------------------------------------------

_POSTGIS_STATEMENTS: list[str] = [
...
async def seed_postgis() -> None:
    """Seed the PostGIS database with fossil sites and paleo-continent data."""
    logger.info("Seeding PostGIS database …")
    for statement in _POSTGIS_STATEMENTS:
        await run_write_query(statement)
    logger.info("PostGIS database seeded successfully.")


# ---------------------------------------------------------------------------
# GraphDB – RDF / Turtle seed
# Loaded into a dedicated named graph so the PUT operation is idempotent
# (it replaces the graph on each seed run).
# ---------------------------------------------------------------------------

_GRAPHDB_NAMED_GRAPH = "http://pangia.io/data/dinosaurs"
...
async def seed_graphdb() -> None:
    """Seed the GraphDB RDF triplestore with the Pangia dinosaur ontology."""
    logger.info("Seeding GraphDB RDF triplestore …")
    await ensure_repository()
    await load_turtle_into_graph(_TURTLE_DATA, _GRAPHDB_NAMED_GRAPH)
    logger.info("GraphDB RDF triplestore seeded successfully.")

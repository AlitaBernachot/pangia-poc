...
def _parse_csv(content: bytes, *, max_rows: int | None = None) -> ParsedFile:
    ...
    # Count total rows (re-read lightweight)
...
def _parse_json(content: bytes, fmt: str, *, max_rows: int | None = None) -> ParsedFile:
    ...
    # Plain JSON — may be a list of records or a wrapped object
    if isinstance(obj, list):
        records = obj
    elif isinstance(obj, dict):
        # Heuristic: look for a list value inside the object
        records = next(
            (v for v in obj.values() if isinstance(v, list)), [obj]
        )
    else:
        records = [{"value": obj}]
...
# Regex detecting a 'lat,lon' or 'lat lon' pair inside a single cell
_LATLON_CELL_RE = re.compile(
    r"(-?\d{1,3}\.\d+)[,\s]+(-?\d{1,3}\.\d+)"
)
    ...
    if not lat_col:
        lat_col = next(
            (orig for lower, orig in cols_lower.items()
             if any(s in lower for s in _LAT_SUBSTRINGS)),
            None,
        )
    if not lon_col:
        lon_col = next(
...
def _find_combined_coord_column(columns: list[str]) -> str | None:
    ...
    return next(
        (orig for lower, orig in cols_lower.items()
         if any(s in lower for s in _COMBINED_SUBSTRINGS)),
        None,
    )
...
def rows_to_geojson(
    ...
    """
    Try to convert a list of row-dicts to a GeoJSON FeatureCollection.
    ...
    if lat_col and lon_col:
        ...
        # ── Combined 'lat,lon' column (e.g. geo_point_2d) ─────────────────

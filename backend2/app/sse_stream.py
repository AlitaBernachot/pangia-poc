...
``stream_graph_events`` drives the compiled LangGraph graph via
``astream_events(version="v2")`` and converts relevant node lifecycle
events into Server-Sent Events for the frontend.
...
async def stream_graph_events(
    ...
    """Stream LangGraph node events as SSE.

    Parameters
    ----------
    graph:
        The compiled orchestrator ``StateGraph``.
    initial_state:
        The initial ``OrchestratorState`` dict for this request.
    original_query:
        The user's original query (needed for ``hitl_request`` SSE event
        so the frontend can display it even after the query may be updated).
    session_id:
        Session identifier used for audit logging.
    """
    audit = get_audit()
    ...
    try:
        async for event in graph.astream_events(initial_state, version="v2"):
            ...
            # ── sub-agent subgraph start ──────────────────────────────────────
                ...
                yield _sse({"type": "agent_start", "agent": node})

            # ── sub-agent subgraph end ────────────────────────────────────────
            elif kind == "on_chain_end" and node in _AGENT_NODE_NAMES:
                out = _output(event)
                # sub_results is the shared key written by post_guardrail_node

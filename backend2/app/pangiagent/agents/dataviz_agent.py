...
The agent returns a structured JSON payload stored in ``AgentOutput.state["dataviz"]``
that the frontend can directly consume to render interactive visualisations.

Note: this agent is called directly inside ``dataviz_node`` after
``humanoutput_node`` has decided visualisations are needed.  It is **not**
part of the router fan-out and is therefore **not** registered in the
``AGENTS`` dict.
"""
...
_DEFAULT_PROMPT = """You are the Data Visualisation Agent of the PangIA GeoIA platform.
...
def build_table(title: str, columns_json: str, rows_json: str) -> str:
    ...
    table: dict[str, Any] = {"title": title, "columns": columns, "rows": rows}
    return json.dumps(table)


_DATAVIZ_TOOLS = [extract_numbers_from_text, build_chart, build_kpi, build_table]
_TOOL_MAP = {t.name: t for t in _DATAVIZ_TOOLS}
...
class DataVizAgent(BaseAgent):
    ...
    called directly inside ``dataviz_node`` after ``humanoutput_node`` has
    determined that dataviz is needed.
    """

    _DEFAULT_PROMPT = _DEFAULT_PROMPT

    def __init__(self, **kwargs) -> None:
        super().__init__(name="dataviz_agent", **kwargs)
        self._system_prompt = self.get_prompt(self._DEFAULT_PROMPT)

    def get_capabilities(self) -> str:
        return (
            "Data Visualisation: detects numerical/statistical data in agent results "
            "and produces chart, KPI, and table structures for the frontend."
        )
    ...
    async def _run(self, inp: AgentInput) -> AgentOutput:
        ...
        # Heuristic: skip entirely if no numeric signal
        if not preseeded_tables and not _NUMERIC_HINT_RE.search(combined_check):
            output = AgentOutput(agent_name=self.name, answer="No visualisable data detected.", confidence=0.5)
            output.state["dataviz"] = None
            return output

        # Short-circuit: table-only mode with preseeded tables
        if preseeded_tables and (table_only_mode or not _NUMERIC_HINT_RE.search(combined_check)):
            dataviz_data: dict[str, Any] = {"charts": [], "kpis": [], "tables": preseeded_tables}
            output = AgentOutput(
                agent_name=self.name,
                answer=f"Visualisations generated: 1 table(s).",
                confidence=0.9,
            )
            output.state["dataviz"] = dataviz_data
            return output

        llm = build_llm(get_agent_model_config(self.name)).bind_tools(
            _DATAVIZ_TOOLS if not table_only_mode else [build_table]
        )
        ...
        messages = [SystemMessage(content=self._system_prompt), HumanMessage(content=dataviz_input)]
        ...
        for _ in range(self.max_iterations):
            ...
            for tc in response.tool_calls:
                ...
                is_error = isinstance(result, str) and result.startswith(
                    ("Unknown tool", "Tool error", "Invalid")
                )
                if not is_error:
                    all_errors = False
                    try:
        ...
        # Build dataviz_data from tool results; fall back to parsing LLM's final message
        dataviz_data = None
        ...
        if collected_charts or collected_kpis or collected_tables:
            ...
            if result_content:
                stripped = re.sub(
                    r"^```(?:json)?\s*|\s*```$", "", result_content.strip(), flags=re.MULTILINE
                )

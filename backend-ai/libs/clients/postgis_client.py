...
Note: asyncpg requires a plain ``postgresql://`` DSN. The ``+asyncpg``
SQLAlchemy prefix is stripped automatically if present.
"""
import json
...
def _normalize_dsn(dsn: str) -> str:
    """Strip the SQLAlchemy ``+asyncpg`` driver prefix from a DSN string."""
...
async def get_pool():
    ...
    if _pool is None:
        ...
        _pool = await asyncpg.create_pool(
            dsn=_normalize_dsn(settings.postgis_dsn),
            min_size=1,
            max_size=5,
        )
    return _pool
...
def _serialize(value):
    """JSON-serialise asyncpg-specific types (Decimal → float, rest → str)."""
